...
# Use slow tokenizer from vocab.txt
...
# =========================================================
# 2) Test data: English + Hindi + Tamil + Telugu
#    Format: (sentence, [(entity_text, entity_label), ...])
# =========================================================
TEST_ITEMS = [
    # ---------------- ENGLISH ----------------
    (
    ...
    (
        "Even some of their office-bearers have joined us, and the BJP is watching closely.",
        [("BJP", "PARTY")]
    ),
    (
...
def normalize_entities(entities):
    return [(clean_text(e_text), e_label) for e_text, e_label in entities]
...
def predict_entities(text: str):
    ...
    using token strings instead of offset mapping.
    """
    enc = tokenizer(
        text,
        return_tensors="pt",
...
for i, (text, gold) in enumerate(TEST_ITEMS, start=1):
    ...
    exact_match = int(gold_set == pred_set)
    sentence_precision = len(gold_set & pred_set) / len(pred_set) if pred_set else 0.0
    sentence_recall = len(gold_set & pred_set) / len(gold_set) if gold_set else 0.0
    sentence_f1 = (
    ...
    print(f"SENTENCE PRECISION: {sentence_precision:.2f}")
    print(f"SENTENCE RECALL:    {sentence_recall:.2f}")
    print(f"SENTENCE F1:        {sentence_f1:.2f}")
...
exact_sentence_accuracy = sum(
    1 for text, gold in TEST_ITEMS
    if set(normalize_entities(gold)) == set(normalize_entities(predict_entities(text)))
) / len(TEST_ITEMS)

print("\n========== FINAL METRICS ==========")
print(f"Entity Accuracy:   {accuracy:.4f}")
print(f"Precision:         {precision:.4f}")
print(f"Recall:            {recall:.4f}")
print(f"F1-score:          {f1:.4f}")
print(f"Sentence Accuracy: {exact_sentence_accuracy:.4f}")